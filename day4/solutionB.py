            ...
            for j in range(5):
                if grid[i][j] == number:
                    ...
                    if (
                        ...
                        completed_grids.append(grid_idx)
...
def main(input_file, output_file):
    ...
    for number in inputs:
        completed_grids = tick(grids, number)
        if len(grids) == 1 and completed_grids:  # last to win & wins
            result = get_result(grids[0], number)
            break
        grids = [
            grids[i] for i in range(len(grids)) if i not in completed_grids
        ]  # filter out winners
    print(result)
